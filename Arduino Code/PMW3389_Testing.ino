...
const int ncs = 10;  //This is the SPI "slave select" pin that the sensor is hooked up,  was 10 is now 53 for mega
...
//Be sure to add the SROM file into this sketch via "Sketch->Add File", instead added to main code as well 
...
void UpdatePointer(void){
  if(initComplete==9){
    ...
    xydat[0] = (int)adns_read_reg(Delta_X_L); //Delta_X_L original 
    xydat[1] = (int)adns_read_reg(Delta_Y_L); //Delta_Y_L original 
...
void loop() {
  ...
  if(movementflag){
    ...
    Serial.print("x = ");
    Serial.print( convTwosComp(xydat[0]) ); //convTwosComp(xydat[0])  original 
    Serial.print(" | ");
    Serial.print("y = ");
    Serial.println( convTwosComp(xydat[1]) ); //convTwosComp(xydat[1]) original 
